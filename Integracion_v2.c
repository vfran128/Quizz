...
void piece_move(int equip, struct piezas *pz, int array_size, int numPiezas,struct pregunta *preguntas  ) {
  ...
  messi:
  printf("elegi como queres moverte\n");
  printf("1) Derecha\n" );
  printf("2) Izquierda\n");
  printf("3) volver a elegir porque claramente te falla el bocho\n");
  scanf("%d", &choice);
  ...
  switch (choice) {
    ...
    if(pz[pieza_actual].x_cord + 1 > numPiezas) {
      ...
      printf("elegi de vuelta.\n");
      goto messi;
    }

    if(equip == 1) {

      if (pz[pieza_actual].y_cord - 1 < 0) {
        printf("No se puede mover.\n");
        printf("elegi de vuelta.\n");
        goto messi;
      }
      ...
      if (pz[pieza_actual].y_cord +   1 > numPiezas) {
        printf("No se puede mover.\n");
        printf("elegi de vuelta.\n");
        goto messi;
      }
    ...
    if (equip == 1) {
      ...
      for (int i = 0; i < numPiezas; i++) {
        if(equip != pz[i].equipo && pz[pieza_actual].x_cord - 1 == pz[i].x_cord && pz[pieza_actual].y_cord - 1 == pz[i].y_cord ) {
          int funcionfrani = Cuestionario(preguntas, 4);
          if(funcionfrani == 0) {
            pz[i].estado = 2;
            pz[pieza_actual].x_cord--;
    ...
    goto label;
    default:
    printf("elegi la opcion correcta simio pelotudo.\n");
    break;
  }
}
